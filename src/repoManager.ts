...
export class RepoManager extends Disposable {
	...
	public getRepoContainingFile(path: string) {
		let repoPaths = Object.keys(this.repos),
			repo = null;
		for (let i = 0; i < repoPaths.length; i++) {
			if (path.startsWith(pathWithTrailingSlash(repoPaths[i])) && (repo === null || repo.length < repoPaths[i].length))
				repo = repoPaths[i];
		}
		return repo;
	}
	...
	public async searchWorkspaceForRepos() {
		...
		if (typeof rootFolders !== 'undefined') {
			for (let i = 0; i < rootFolders.length; i++) {
				if (await this.searchDirectoryForRepos(getPathFromUri(rootFolders[i].uri), this.maxDepthOfRepoSearch))
					changes = true;
			}
		}
					...
					if (root !== null) {
						...
						fs.readdir(directory, async (err, dirContents) => {
							if (err) {
								...
								resolve(
									(await evalPromises(dirs, 2, (dir) => this.searchDirectoryForRepos(dir, maxDepth - 1))).indexOf(
										true
									) > -1
								);
							}
		...
		fs.mkdir(vscodePath, (err) => {
			if (!err || err.code === 'EEXIST') {
				...
				fs.writeFile(configPath, JSON.stringify(file, null, 4), (err) => {
					if (err) {
						reject(
							'Failed to write the Git Graph Repository Configuration File to "' + getPathFromStr(configPath) + '".'
						);
					} else {
						resolve(
							'Successfully exported the Git Graph Repository Configuration to "' + getPathFromStr(configPath) + '".'
						);
					}
				});
			} else {
...
function validateExternalConfigFile(file: Readonly<ExternalRepoConfig.File>) {
	...
	if (
		...
		(!Array.isArray(file.hideRemotes) || file.hideRemotes.some((remote) => typeof remote !== 'string'))
	) {
		return 'hideRemotes';
	}
	if (
