...
export class DataSource extends Disposable {
	...
	public async fetchIntoLocalBranch(
		...
		const currentBranch = await this.spawnGit(['symbolic-ref', '--short', 'HEAD'], repo, (stdout) =>
			stdout.trim(),
		);
	...
	public async editCommitMessage(
		...
		try {
			const headCommit = await this.spawnGit(['rev-parse', 'HEAD'], repo, (stdout) =>
				stdout.trim(),
			);
	 ...
	 * @returns The ErrorInfo from the executed command.
	 */
	private async rebaseEditCommitMessage(
		repo: string,
		commitHash: string,
		message: string,
	): Promise<ErrorInfo> {
		const parentCommit = await this.spawnGit(['rev-parse', commitHash + '^'], repo, (stdout) =>
			stdout.trim(),
		);
			...
			if (this.gitExecutable === null) {
				...
				if (isGui) {
					...
					openGitTerminal(
						...
						args.join(' '),
						vscode.l10n.t('ui.openExternalDirDiff'),
					);
				}
				...
				if (squashMessageFormat === SquashMessageFormat.Default) {
					args.push(
						...
						(actionOn === MergeActionOn.Branch
							? vscode.l10n.t('ui.mergeBranch')
							: vscode.l10n.t('ui.mergeCommit')) +
							" '" +
							obj +
							"'",
					);
