...
class TextFormatter {
		...
		if (this.config.markdown) {
			...
			while ((match = TextFormatter.EMPHASIS_REGEXP.exec(input))) {
				...
				while (cur < seq.length - 1) {
					...
					isLeft =
						!whitespaceAfter && (!punctuationAfter || (punctuationAfter && (whitespaceBefore || punctuationBefore)));
					isRight =
						!whitespaceBefore && (!punctuationBefore || (punctuationBefore && (whitespaceAfter || punctuationAfter)));
			...
			for (let i = 0; i < emphasisTokens.length; i++) {
				...
				if (
					run.close &&
					(stackMatch = TextFormatter.findOpenEmphasis(delimiter, run, emphasisRuns, emphasisStack)) > -1
				) {
					TextFormatter.insertIntoTree(tree, {
						type:
	...
	public static registerCustomEmojiMappings(mappings: ReadonlyArray<GG.CustomEmojiShortcodeMapping>) {
		...
		for (let i = 0; i < mappings.length; i++) {
			if (validShortcodeRegExp.test(mappings[i].shortcode)) {
				TextFormatter.EMOJI_MAPPINGS[mappings[i].shortcode.substring(1, mappings[i].shortcode.length - 1)] =
					mappings[i].emoji;
			}
		}
	}
	...
	private static combineNestedEmphasis(tree: TF.Node) {
		...
		if (
			...
			tree.type = tree.type === TF.NodeType.Asterisk ? TF.NodeType.DoubleAsterisk : TF.NodeType.DoubleUnderscore;
