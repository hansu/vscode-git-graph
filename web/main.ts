class GitGraphView {
      ...
      (!this.currentRepoRefreshState.hard &&
		...
		const filterCurrentBranches = () => {
			...
			if (this.currentBranches !== null && !(this.currentBranches.length === 1 && this.currentBranches[0] === SHOW_ALL_BRANCHES)) {
				...
				this.currentBranches = this.currentBranches.filter((branch) => this.gitBranches.includes(branch) || globPatterns.includes(branch) || branch === 'HEAD');
			}
		};
	...
	private finaliseRepoLoad(didLoadRepoData: boolean) {
        ...
        (this.expandedCommit === null ||
          ...
          this.loadViewTo.commitDetails.compareWithHash !== null ? this.getCommitId(this.loadViewTo.commitDetails.compareWithHash) : null;
   ...
   */
	...
	private requestLoadCommits() {
		...
		sendMessage({
			...
			branches:
        this.currentBranches === null || (this.currentBranches.length === 1 && this.currentBranches[0] === SHOW_ALL_BRANCHES) ? null : this.currentBranches,
	...
	private renderTable() {
      ...
      '</tr>';
        ...
        (this.config.referenceLabels.branchLabelsAlignedToGraph
        	? '<td>' +
            getResizeColHtml(0) +
            (refBranches !== '' ? '<span style="margin-left:' + (widthsAtVertices[i] - 4) + 'px"' + refBranches.substring(5) : '') +
            '</td><td>' +
            getResizeColHtml(1) +
            '<span class="description">' +
            commitDot
        	: '<td>' + getResizeColHtml(0) + '</td><td>' + getResizeColHtml(1) + '<span class="description">' + commitDot + refBranches) +
        ...
        (colVisibility.author
            ...
            (this.config.fetchAvatars
                ...
                '">' +
                (typeof this.avatars[commit.email] === 'string' ? '<img class="avatarImg" src="' + this.avatars[commit.email] + '">' : '') +
                '</span>'
            	: '') +
	...
	private checkoutBranchAction(refName: string, remote: string | null, prefillName: string | null, target: DialogTarget & (CommitTarget | RefTarget)) {
		if (remote !== null) {
			dialog.showRefInput(
				...
				(newBranch) => {
					if (this.gitBranches.includes(newBranch)) {
						...
						dialog.showTwoButtons(
							...
							() => {
								runAction(
									...
									canPullFromRemote ? getText('ui.actionCheckingOutBranchAndPullingChanges') : getText('ui.actionCheckingOutBranch')
								);
							},
							target
						);
						...
						runAction(
							{ command: 'checkoutBranch', repo: this.currentRepo, branchName: newBranch, remoteBranch: refName, pullAfterwards: null },
			...
			runAction({ command: 'checkoutBranch', repo: this.currentRepo, branchName: refName, remoteBranch: null, pullAfterwards: null }, getText('ui.actionCheckingOutBranch'));
		}
	}
		...
		dialog.showForm(
			...
			(values) => {
				...
				if (this.gitBranches.includes(branchName)) {
					...
					runAction(
						{ command: 'createBranch', repo: this.currentRepo, branchName: branchName, commitHash: hash, checkout: checkOut, force: false },
						getText('ui.actionCreatingBranch')
					);
				}
			},
	...
	private deleteTagAction(refName: string, deleteOnRemote: string | null) {
		runAction({ command: 'deleteTag', repo: this.currentRepo, tagName: refName, deleteOnRemote: deleteOnRemote }, getText('ui.actionDeletingTag'));
	}

	private fetchFromRemotesAction() {
		runAction(
			{ command: 'fetch', repo: this.currentRepo, name: null, prune: this.config.fetchAndPrune, pruneTags: this.config.fetchAndPruneTags },
	...
	private rebaseAction(obj: string, name: string, actionOn: GG.RebaseActionOn, target: DialogTarget & (CommitTarget | RefTarget)) {
        ...
        '</i></b>?',
			...
			(values) => {
				...
				runAction(
					{ command: 'rebase', repo: this.currentRepo, obj: obj, actionOn: actionOn, ignoreDate: <boolean>values[1], interactive: interactive },
	...
	private makeTableResizable() {
		...
		colHeadersElem.addEventListener('contextmenu', (e: MouseEvent) => {
			...
			const toggleColumnState = (col: number, defaultWidth: number) => {
				columnWidths[col] =
          columnWidths[col] !== COLUMN_HIDDEN ? COLUMN_HIDDEN : columnWidths[0] === COLUMN_AUTO ? COLUMN_AUTO : defaultWidth - COLUMN_LEFT_RIGHT_PADDING;
				this.saveColumnWidths(columnWidths);
				this.render();
			};
   ...
   */
	private scrollToStash(next: boolean) {
        ...
        this.lastScrollToStash.hash !== null ? stashCommits.findIndex((commit) => commit.hash === this.lastScrollToStash.hash) : -1;
   ...
   */
	...
	private observeUrls() {
		const followInternalLink = (e: MouseEvent) => {
			if (e.target !== null && isInternalUrlElem(<Element>e.target)) {
              ...
              (this.expandedCommit === null || this.expandedCommit.commitHash !== value || this.expandedCommit.compareWithHash !== null)
						) {
			...
			if ((eventElem = eventTarget.closest('.gitRef')) !== null) {
				...
				if (eventElem.classList.contains(CLASS_REF_HEAD) || eventElem.classList.contains(CLASS_REF_REMOTE)) {
					...
					this.checkoutBranchAction(
						refName,
						isHead ? null : unescapeHtml((isRemoteCombinedWithHead ? <HTMLElement>eventTarget : eventElem).dataset.remote!),
						null,
						target
					);
				}
	...
	private renderCommitDetailsView(refresh: boolean) {
      ...
      !expandedCommit.loading && (expandedCommit.compareWithHash !== null || this.commits[this.commitLookup[expandedCommit.commitHash]].parents.length > 0);
      ...
      '</div><div class="cdvHeightResize"></div>';
		...
		elem.innerHTML = isDocked
			? html
			: '<td><div class="cdvHeightResize"></div></td><td colspan="' + (this.getNumColumns() - 1) + '"><div id="cdvContentWrapper">' + html + '</div></td>';
		this.setCdvDivider();
		this.setCdvHeight(elem, isDocked);
          ...
          isExtensionErrorInfo(msg.errors[1], GG.ErrorInfoExtensionPrefix.PushTagCommitNotOnRemote)
			...
			case 'checkoutBranch':
				refreshAndDisplayErrors(
					msg.errors,
					msg.pullAfterwards !== null ? getText('ui.unableToCheckoutBranchAndPullChanges') : getText('ui.unableToCheckoutBranch')
				);
				break;
			case 'checkoutCommit':
			...
			case 'deleteUserDetails':
				finishOrDisplayErrors(msg.errors, getText('ui.unableToRemoveGitUserDetails'), () => gitGraph.requestLoadConfig(), true);
				break;
			case 'dropCommit':
			...
			case 'editUserDetails':
				finishOrDisplayErrors(msg.errors, getText('ui.unableToSaveGitUserDetails'), () => gitGraph.requestLoadConfig(), true);
				break;
...
function getShowRemoteBranches(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.showRemoteBranches : repoValue === GG.BooleanOverride.Enabled;
}

function getSimplifyByDecoration(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.simplifyByDecoration : repoValue === GG.BooleanOverride.Enabled;
}

function getShowStashes(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.showStashes : repoValue === GG.BooleanOverride.Enabled;
}

function getShowTags(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.showTags : repoValue === GG.BooleanOverride.Enabled;
}

function getIncludeCommitsMentionedByReflogs(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.includeCommitsMentionedByReflogs : repoValue === GG.BooleanOverride.Enabled;
}

function getOnlyFollowFirstParent(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.onlyFollowFirstParent : repoValue === GG.BooleanOverride.Enabled;
}

function getOnRepoLoadShowCheckedOutBranch(repoValue: GG.BooleanOverride) {
	return repoValue === GG.BooleanOverride.Default ? initialState.config.onRepoLoad.showCheckedOutBranch : repoValue === GG.BooleanOverride.Enabled;
}
...
function haveFilesChanged(oldFiles: ReadonlyArray<GG.GitFileChange> | null, newFiles: ReadonlyArray<GG.GitFileChange> | null) {
	if ((oldFiles === null) !== (newFiles === null)) {
      ...
      newFiles!,
      (a, b) =>
      	a.additions === b.additions && a.deletions === b.deletions && a.newFilePath === b.newFilePath && a.oldFilePath === b.oldFilePath && a.type === b.type
		);
	}
}
...
function rebaseUnableToErrorTitle(actionOn: GG.RebaseActionOn): string {
	return actionOn === GG.RebaseActionOn.Commit ? getText('ui.unableToRebaseOnCommit') : getText('ui.unableToRebaseOnBranch');
}
