...
class SettingsWidget {
				...
				if (pullRequestConfig !== null) {
					...
					const destination = escapeHtml(
						pullRequestConfig.destOwner +
							...
							(pullRequestConfig.destRemote !== null ? ' (' + pullRequestConfig.destRemote + ')' : '')
					);
		...
		dialog.showForm(
			...
			(values) => {
				...
				if (config.sourceRemote !== newSourceRemote) {
					...
					const match =
						newSourceUrl !== null ? newSourceUrl.match(/^(https?:\/\/|git@)[^/:]+[/:]([^/]+)\/([^/]*?)(.git|)$/) : null;
				...
				if (config.destRemote !== newDestRemote) {
					...
					if (newDestRemote !== null) {
						const match =
							newDestUrl !== null ? newDestUrl.match(/^(https?:\/\/|git@)[^/:]+[/:]([^/]+)\/([^/]*?)(.git|)$/) : null;
						...
						const branches = this.view
							...
							.filter(
								(branch) =>
									branch.startsWith('remotes/' + newDestRemote + '/') && branch !== 'remotes/' + newDestRemote + '/HEAD'
							)
	...
	private showCreatePullRequestIntegrationDialog2(config: GG.DeepWriteable<GG.PullRequestConfig>) {
		...
		const updateConfigWithFormValues = (values: DialogInputValue[]) => {
			...
			config.destBranch =
				config.destRemote === null || destBranches.length === 0 ? destBranch : destBranches[parseInt(destBranch)];
		};
