...
class SettingsWidget {
	...
	private render() {
		if (this.currentRepo !== null && this.repo !== null) {
			...
			if (this.config !== null) {
				...
				if (userNameSet || userEmailSet) {
					const escapedUserName = escapeHtml(
						userName.local ?? userName.global ?? getText('ui.notSet'),
					);
					const escapedUserEmail = escapeHtml(
						userEmail.local ?? userEmail.global ?? getText('ui.notSet'),
					);
					html +=
						'<table>' +
						'<tr><td class="left">' +
						...
						escapedUserName +
						(userNameSet
							? ' (' + (userName.local !== null ? getText('ui.local') : getText('ui.global')) + ')'
							: '') +
						'">' +
						escapedUserName +
						'</td></tr>' +
						...
						escapedUserEmail +
						(userEmailSet
							? ' (' + (userEmail.local !== null ? getText('ui.local') : getText('ui.global')) + ')'
							: '') +
						'">' +
						escapedUserEmail +
						'</td></tr>' +
				...
				if (this.config.remotes.length > 0) {
					...
					html +=
						'<tr class="lineAbove"><td colspan="4">' +
						getText('ui.noRemotesConfigured') +
						'</td></tr>';
				}
				html +=
			...
			if (this.config !== null) {
				html +=
					'<div class="settingsSection centered"><h3>' +
					getText('ui.pullRequestCreation') +
					'</h3>';
						...
						getText('ui.areYouSureYouWantToRemove') +
							(locallyConfigured
								? (globalState.issueLinkingConfig !== null
										? getText('ui.theLocallyConfiguredInThisRepository')
		...
		this.loadingElem.innerHTML = this.loading
			? '<span>' + SVG_ICONS.loading + getText('ui.loading') + '</span>'
			: '';
	...
	private showIssueLinkingDialog(
		...
		let html =
			'<b>' +
			(isEdit
				? getText('ui.editIssueLinkingForThisRepository')
				: getText('ui.addIssueLinkingForThisRepository')) +
			'</b>';
		html +=
